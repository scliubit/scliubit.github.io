...
> 这里还有件十分有趣的事情。即便学术环境看起来糟糕了许多，但是海外还是有不少老师在做一些靠谱的研究。当这个很小丑的「电磁信息论」的概念还没出现的时候，海外有些老师在利用一些「全息通信」、「连续孔径」等概念有意缓慢逼近一些光学的前期成果。
...
> 当然实际上「语义」和「电磁信息论」的底层思想是相同的。信息论指导信息的压缩，而语义则是希望从另一个维度进一步压缩，实现「Beyond Shannon」。举个例子，我们对一段话编码压缩主要是依靠传统的香农熵理论（无损编码，例如算数编码、霍夫曼编码等），如果我们只看Hello这个词，
