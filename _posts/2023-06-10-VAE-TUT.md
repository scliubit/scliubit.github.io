...
class Encoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> List[torch.Tensor]:
        ...
        return [self.act_mu(mu), self.act_var(log_var)]
...
class Decoder(nn.Module):
    def __init__(
        ...
        self.latent_dim = latent_dim
        self.hidden_dims = hid_dim
        self.fc1 = nn.Linear(latent_dim, hid_dim).to(device)
        self.dropout = nn.Dropout(0.0)
        self.activation = nn.Tanh()
        self.fc2 = nn.Linear(hid_dim, 784).to(device)
        self.act2 = nn.Sigmoid()

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = self.activation(self.fc1(x))
        x = self.fc2(x)
        return self.act2(x).view(-1, 1, 28, 28)
...
class VAE(nn.Module):
    def __init__(
        ...
        self.latent_dim = latent_dim
        self.hidden_dims = hid_dim
        self.encoder = Encoder(hid_dim, latent_dim, device).to(device)
        self.decoder = Decoder(hid_dim, latent_dim, device).to(device)
        self.device = device
    ...
    def loss_function(
        ...
        kld_weight: float = 0.003,
    ) -> List[torch.Tensor]:
        MSE = F.mse_loss(recon_x.view(-1, 784), x.view(-1, 784), reduction="mean")
        KLD = torch.mean(
            -0.5 * torch.sum(1 + log_var - mu.pow(2) - log_var.exp(), dim=1)
        )
        return [MSE + KLD * kld_weight, MSE, KLD]


def show_image_grid(imgs, num_images=64, size=(1, 28, 28)):
    ncols = int(np.sqrt(num_images))
    nrows = int(np.sqrt(num_images))
    fig, ax = plt.subplots(nrows, ncols, figsize=(ncols, nrows))
    for i in range(ncols):
        for j in range(nrows):
            ax[i, j].imshow(imgs[i * nrows + j].reshape(size[1], size[2]), cmap="gray")
            ax[i, j].axis("off")
    plt.show()
...
鉴于本文首次提出VAE的时间是2013年，此时还没有CNN类模型的大热，残差网络结构还未正式提出，因此一个修改后的版本可以在[这里]()下载
