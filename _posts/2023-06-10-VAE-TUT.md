...
左侧是`证据`的似然函数，因此右侧得名为证据下界(`Evidence Lower BOund, ELBO`)，有时也被称为是变分下界[^variationallb]。显然如果我们想要让两个分布接近就需要优化KL散度，而由于证据为常数，因此最小化KL散度就等价为最大化证据下界，这样就避免了直接计算后验概率的问题。
