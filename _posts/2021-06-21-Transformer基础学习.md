...
需要注意的是，对于输入而言，如果序列长度不统一，则在输入之前会统一padding到统一的长度（某最大长度约束）。多层EncoderLayer叠层之后将构成Encoder结构。
...
相比EncoderLayer，DecoderLayer的结构并没有较大的改动。其基本处理方式与EncoderLayer相同，均为前馈网络与并行注意力的结合。然而根据Encoder和Decoder的图示结构容易知道，Decoder中相比Encoder将增加一个对目标序列处理的模块。Decoder接收两部分输入，分别是从Encoder的输出（也被称为是Memory）和目标序列（也被称为是target）。target序列首先通过自注意力进行预处理，然后与memory内容结合进行后续与Encoder类似的处理。
