...
![](/img/in-post/holo/array.png){:height="55%" width="55%"}
...
该积分的处理稍有些复杂，可以根据我们的实际情况做合理化简，例如此时波长很小，很容易满足远场条件，则光屏接收到的光强信号可以表示为
...
其中$s(X,Y) = (-i/\lambda z )e^{i\pi(x^2+y^2)/\lambda z}$。采样过程可以看做是傅里叶变换，其实重建过程也类似，因为二重积分的形式相同。
