...
> 作为这个页面搭建后的第一篇文章，说实话还是有占位的嫌疑

# 简介

我们常听说电磁波的衰减遵循频率（或距离）的平方反比原则，这里其实就是引用了**Friis**在自由空间传输的数学模型[^1]，其形式如下所示

$$\begin{align}P_r/P_t=A_rA_t/d^2\lambda^2\tag{1},\label{eq}\end{align}$$
...
<html><div align="center"><img src="./img/Friis/sysModel.png" title="system model" alt="MIMO" height="263" width="726" style="display:block; margin:0 auto;"></div></html>

此时单位面积接收到的信号功率为
$$
\begin{align}
P_0&=P_t/A_{\rm eff.}\\&=P_t/4\pi d^2,\tag{3}
\end{align}
$$
带入考虑各向同性天线的孔径，以及公式$(\ref{aper})$，容易得到
$$
\begin{align}
P_r/P_t&=A_r/4\pi d^2\\&=A_rA_t/4\pi d^2A_{\rm isotr.}\\&=A_rA_t/d^2\lambda^2,\tag{4}
\end{align}
$$
...
$$
\begin{align}
p=\frac{P_T}{4\pi R^2},\tag{5}
\end{align}
$$
如果假设发射天线的天线增益（方向增益），则可以考虑结果为
$$
\begin{align}
p=\frac{P_T}{4\pi R^2}G_T,\tag{6}
\end{align}
$$
假设接收天线的等效孔径为$A_{ER}$
$$
\begin{align}\
P_R=pA_{ER}=\frac{P_T}{4\pi R^2}G_T A_{ER},\tag{7}
\end{align}
$$
带入各向同性天线的等效孔径$A_{\rm eff.}=\lambda^2/4\pi$（事实上，如果我们以此为基础计算增益系数$G$，对于任何天线都可以得到等效孔径为$A_{\rm eff.}^*=G \lambda^2/4\pi $）可以得到
$$
\begin{align}\
P_R=pA_{ER}=\frac{P_T G_T G_R \lambda^2}{(4\pi R)^2},\tag{8}
\end{align}
$$
或
$$
\begin{align}\
P_R=pA_{ER}=\frac{P_T G_T G_R c^2 }{(4\pi Rf)^2},\tag{8}
\end{align}
$$
...
\end{align}
$$
其中$a$是天线的最大线性维度（例如$16\times 32$的UPA中，$a=32$）。
